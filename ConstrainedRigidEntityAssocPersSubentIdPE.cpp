...
public:
    ...
    // Called from ConstrainedRigidEntityAssocPersSubentIdPE::notifyDragStatus()
    //
    void notifyDragStatus(const AcDbEntity* pOrigEntity, AcDb::DragStat dragStatus);
    ...
    //
    void notifyCopied(const AcDbEntity* pOrigEntity, AcDbEntity* pClone);

    // Called from ConstrainedRigidEntityAssocPersSubentIdPE::notifyDeleted().
    // It only reacts to the non-database-resident entity clones
    //
    void notifyDeleted(const AcDbEntity*);
    ...
    // It only caches the explcitly requested subentnties, not all subentities
    ...
    //
    // Remove entries whose dragging has ended (their DragState is kDragEnd) and that
    // only have constraints from the database of this top-level network, i.e.
    // all their constraints have been solved.
    //
    // If the entity changed shape during dragging, request one more evaluation to 
    // statically evaluate the constraints with the changed entity geometry
    //
    void doneWithTopLevelNetworkEvaluation(const AcDbAssocNetwork* pTopLevelNetwork);
...
private:
    ...
    public:
        ...
        const AcDbEntity*     mpEntityClone; // Becomes a dangling pointer, after the entity clone has been deleted
        AcGeMatrix3d          mOrigRigidSetTransform;
        AcArray<AcDbSubentId> mOrigVertexSubentIds;
        AcArray<AcDbSubentId> mOrigEdgeSubentIds;
    ...
    int findEntityEntry(const AcDbEntity*) const; // The entity may either be the original entity or a clone
    void removeEntry(const AcDbEntity* pOrigEntity, int index);
...
void DraggedConstrainedRigidEntitiesReactor::copied(const AcDbObject* pDbObj, const AcDbObject* pNewObj)
{
    ...
    sCurrentlyDragged.notifyCopied(AcDbEntity::cast(pDbObj), AcDbEntity::cast(pNewObj));
}
...
void DraggedConstrainedRigidEntitiesEvalCallback::endActionEvaluation(AcDbAssocAction* pAction)
{
    ...
    sCurrentlyDragged.doneWithTopLevelNetworkEvaluation(pNetwork);
}
...
void DraggedConstrainedRigidEntities::removeEntry(const AcDbEntity* pOrigEntity, int index)
{
    if (index != -1)
    {
...
void DraggedConstrainedRigidEntities::notifyDragStatus(const AcDbEntity* pOrigEntity, AcDb::DragStat dragStatus)
{
    ...
    case AcDb::kDragStart:
        {
            if (index != -1)
            {
                assert(!"The entity is already monitored");
                removeEntry(pOrigEntity, index);
            }
            ...
            {
                const int kTooMany = 100;

                if (mEntityEntries.length() >= kTooMany)
                {
                    assert(!"Too many dragged constrained rigid entities (or a bug in this code)");
    ...
    case AcDb::kDragEnd:
        {
            ...
            {
                // The dragged entity was not remembered as it has no dependencies
                // from a contraint group
            }
        }
        break;
...
//
void DraggedConstrainedRigidEntities::notifyDeleted(const AcDbEntity* pEntity)
{
...
void DraggedConstrainedRigidEntities::doneWithTopLevelNetworkEvaluation(const AcDbAssocNetwork* pNetwork)
{
    for (int i = mEntityEntries.length()-1; i >= 0; i--)
    {
        EntityEntry* const pEntityEntry = mEntityEntries[i];
        if (pEntityEntry->mDragStat != AcDb::kDragEnd)
            continue;
        ...
        for (int j = pEntityEntry->mConstraintGroupIds.length()-1; j >= 0; j--)
        {
            ...
            {
                pEntityEntry->mConstraintGroupIds.removeAt(j); // Done with this constraint group
            }
        }
        ...
        if (pEntityEntry->mConstraintGroupIds.isEmpty()) 
        {
            if (pEntityEntry->mEntityChangedShape)
            {
                ...
                //
                for (int j = 0; j < pEntityEntry->mConstraintDepIds.length(); j++)
                {
...
void DraggedConstrainedRigidEntities::cacheOrigSubentGeometry(const AcDbEntity* pEntity, const AcDbSubentId& subentId)
{
    ...
    case AcDb::kEdgeSubentType:
        if (!pEntityEntry->mOrigEdgeSubentIds.contains(subentId))
        {
            AcDbObjectPointer<AcDbEntity> pOrigEntity(pEntityEntry->mOrigEntityId, AcDb::kForRead);
            ...
            pEntityEntry->mOrigEdgeSubentIds.append(subentId);
            pEntityEntry->mOrigEdgeSubentGeometries.append(pEdgeCurve);
        }
        break;
    case AcDb::kVertexSubentType:
        if (!pEntityEntry->mOrigVertexSubentIds.contains(subentId))
        {
            AcDbObjectPointer<AcDbEntity> pOrigEntity(pEntityEntry->mOrigEntityId, AcDb::kForRead);
            if (!eOkVerify(pOrigEntity.openStatus()))
                return;
            ...
            pEntityEntry->mOrigVertexSubentIds.append(subentId);
                                                                                ...
                                                                                AcGeCurve3d*&       pEdgeCurve) const
{
    ...
    if (index == -1)
    {
        assert(!"Entity not monitored");
        return Acad::eInvalidInput;
    }
    const EntityEntry* const pEntityEntry = mEntityEntries[index];
    const int i = pEntityEntry->mOrigEdgeSubentIds.find(edgeSubentId);
    if (i == -1)
    {
                                                                                  ...
                                                                                  AcGePoint3d&        vertexPosition) const
{
    ...
    const EntityEntry* const pEntityEntry = mEntityEntries[index];
    const int i = pEntityEntry->mOrigVertexSubentIds.find(vertexSubentId);
    if (i == -1)
    {
...
void DraggedConstrainedRigidEntities::notifyCopied(const AcDbEntity* pOrigEntity, AcDbEntity* pEntityClone)
{
    const AcDbObjectId origEntityId = pOrigEntity->objectId();
    if (origEntityId.isNull())
...
void ConstrainedRigidEntityAssocPersSubentIdPE::notifyDragStatus(const AcDbEntity* pOrigEntity, AcDb::DragStat dragStatus)
{
    sCurrentlyDragged.notifyDragStatus(pOrigEntity, dragStatus);
}


void ConstrainedRigidEntityAssocPersSubentIdPE::notifyDeleted(const AcDbEntity* pEntity)
{
    sCurrentlyDragged.notifyDeleted(pEntity);
}
