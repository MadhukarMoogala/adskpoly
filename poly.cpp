...
static Acad::ErrorStatus drawFacets(const AsdkPoly*         poly,
										  ...
										  AcGiViewportDraw* vportDraw, Adesk::Int32 numSides);
...
AsdkPoly::~AsdkPoly()
{
	sPolyAssocPersSubentIdPE.notifyDeleted(this); // For Constraints
									  ...
									  AcDbObjectId*        entAndInsertStack) const
{
	...
	// The text subentity isn't a vertex or edge, so we don't return it here.
	//
	if (gsMark == mNumSides + 1)
		return Acad::eInvalidInput;

	const int index = (int)gsMark - 1;
	...
	case AcDb::kVertexSubentType:
		...
		subentPaths[0] = AcDbFullSubentPath(objectId(), AcDb::kVertexSubentType, index);
		subentPaths[1] = AcDbFullSubentPath(objectId(), AcDb::kVertexSubentType, (index+1) % mNumSides);
		break;
	case AcDb::kEdgeSubentType:
		numPaths       = 1;
		subentPaths    = new AcDbFullSubentPath[1];
		subentPaths[0] = AcDbFullSubentPath(objectId(), AcDb::kEdgeSubentType, index);
		break;
	default:
		;
	}
									  ...
									  AcDbIntPtrArray&          gsMarkers) const
{
	...
	case AcDb::kVertexSubentType:
		if (index < 0)
			return Acad::eOk; // No Gs marker for the center and text vertex subentities
...
AsdkPoly::subSubentPtr(const AcDbFullSubentPath& path) const
{
	...
	case AcDb::kVertexSubentType:
		{
			AcDbPoint* const pPoint = new AcDbPoint();
			pPoint->setPosition(vertexCoords[index-1]);
	...
	case AcDb::kEdgeSubentType:
		{
			AcDbLine* const pLine = new AcDbLine();
			pLine->setStartPoint(vertexCoords[index-1]);
			pLine->setEndPoint(vertexCoords[index]);
	...
	AcDbIntArray&         /*geomIds*/) const
{
	...
	if (gsSelectionMark == -1) //text Geomerty
		{
...
AsdkPoly::subWorldDraw(AcGiWorldDraw* worldDraw)
{
	...
	case kAcGiShadedDisplay:
		...
		drawFacets(this, worldDraw, NULL, mNumSides);
		break;
...
{
	sPolyAssocPersSubentIdPE.notifyDragStatus(this, dragStatus); // For Constraints
}
										  ...
										  AcGiViewportDraw* vportDraw, Adesk::Int32 numSides)
{
	...
	AcGiEdgeData* edgeData = new AcGiEdgeData();
	int numEdges = numSides;
	short *pEdgeColorArray = new short[numEdges];
	for (int i = 0; i < numEdges; i++)
	{
		pEdgeColorArray[i] = kWhite;
	}

	AcGiFaceData* faceData = new AcGiFaceData();
	short *sOrigColors = faceData->colors();
	static short testColors[] = { kWhite };

	faceData->setColors(testColors);
										 ...
										 AcGiViewportDraw* vportDraw)
{
	...
	worldDraw->subEntityTraits().setFillType(AcGiFillType::kAcGiFillAlways);
    worldDraw->geometry().polygon(vertexArray.length(), vertexArray.asArrayPtr());
...
double fixangle(double angle)
{
	...
	retang = angle / TWOPI;
	retang = (retang - ((int)(retang - 0.5))) * TWOPI;
...
double distan2(const AcGePoint3d& pt1, const AcGePoint3d& pt2)
{
	return sqrt((pt1.x - pt2.x) * (pt1.x - pt2.x) + (pt1.y - pt2.y) * (pt1.y - pt2.y) + (pt1.z - pt2.z) * (pt1.z - pt2.z));
}
void rswap(double &ra, double &rb)
{
	double t;
	t = ra;
	ra = rb;
...
void vertexEdgeToArc(ads_point p1, ads_point p2, ads_real bulge, ads_point cenp,
	ads_real& radius, ads_real& starta, ads_real& enda)
{
	ads_real cotbce;

	cotbce = 0.5 * (1.0 / bulge - bulge);
	cenp[X] = 0.5 * (p1[X] + p2[X] - (p2[Y] - p1[Y]) * cotbce);
	cenp[Y] = 0.5 * (p1[Y] + p2[Y] + (p2[X] - p1[X]) * cotbce);
	radius = distan2(asPnt3d(cenp), asPnt3d(p1));
	starta = fixangle(atan2(p1[Y] - cenp[Y], p1[X] - cenp[X]));
	enda = fixangle(atan2(p2[Y] - cenp[Y], p2[X] - cenp[X]));
	if (bulge < 0.0)
		rswap(starta, enda);
}
...
Acad::ErrorStatus acdbMakeArc(const AcGePoint3d&   pt1,
	...
	AcGeCircArc3d&      arc)
{
	...
	AcGePoint3d center;
	double radius, starta, enda;

	// Initialize the Z of the center with Z from pt1, to ensure
	// the arc will be created with the proper orientation.
	
	center.z = pt1.z;

	// This function will return starta and enda always in the first
	// 2PI.  If the bulge is negative (i.e. clockwise arc) then the
	// function will swap the start & end angles before returning, so
	// that the start angle will *always* be correct for a counterclockwise
	// arc segment.  However,  since the angles are in the first 2PI, it is
	// still possible for the start angle to be greater than the end angle
	// which, contrary to the AcGe docs, is ok because the arc set method
	// used below will add whatever multiple of 2PI is necessary to the
	// end angle to make it larger than the start angle.
	//
	vertexEdgeToArc(asDblArray(pt1), asDblArray(pt2), bulge, asDblArray(center), radius, starta, enda);

	acdbEcs2Wcs(asDblArray(center), asDblArray(center),
		asDblArray(entNorm), false);
	arc.set(center, entNorm, entNorm.perpVector(), radius, starta, enda);

	return es;
}
void Test1()
{
	AcGePoint3d   pt1(3.0, 2.0, 0.0);
	AcGePoint3d  pt2(4.0, 4.0, 0.0);
	double bulge = -0.5;
	AcGeVector3d entNorm(0.0, 0.0, 1.0);
	AcGeCircArc3d arc;
	acdbMakeArc(pt1, pt2, bulge, entNorm, arc);
	AcDbCurve* pArc = nullptr;
	AcDbObjectId iArcId = AcDbObjectId::kNull;
	AcDbCurve::createFromAcGeCurve(arc, (AcDbCurve*&)pArc);
	...
	pBlockTableRecord->appendAcDbEntity(iArcId, pArc);
...
acrxEntryPoint(AcRx::AppMsgCode msg, void* pkt)
{
	...
	case AcRx::kInitAppMsg:
	  ...
	  //  AsdkPoly::rxInit(changeAppNameCallback);
		...
		acedRegCmds->addCommand(_T("ASDK_POLYGON"), _T("ASDK_TEST"), _T("MkArc"), ACRX_CMD_MODAL, &Test1);
		break;
	...
	case AcRx::kUnloadAppMsg:
		...
		acrxBuildClassHierarchy();
		acedRegCmds->removeCmd(_T("ASDK_POLYGON"),_T("ASDK_POLY"));
		acedRegCmds->removeCmd(_T("ASDK_POLYGON"), _T("ASDK_TEST"));
