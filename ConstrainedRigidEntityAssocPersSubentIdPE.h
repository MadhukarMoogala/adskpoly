...
//
// The custom entity also needs to call the follwing methods of the 
...
//   Must be called when the custom entity shape changes (including scaling and 
//   mirror). When the whole entity is transformed by a rigid transform 
...
public:
    ...
    // derived from AcDbAssocPersSubentIdPE
    // 
    ...
    //
    void notifyDragStatus        (const AcDbEntity* pOrigEntity, AcDb::DragStat); // Call from the entity's dragStatus() method
    void notifyEntityChangedShape(const AcDbEntity*);                             // Call when the entity shape changes
    void notifyDeleted           (const AcDbEntity*);                             // Call from the entity's destructor
